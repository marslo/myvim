...
function! python_ftplugin#misc#option#get(name, ...) " {{{1
...
function! python_ftplugin#misc#option#split(value) " {{{1
  " Given a multi-value Vim option like ['runtimepath'] [rtp] this returns a
  " list of strings. For example:
  "
  "     :echo python_ftplugin#misc#option#split(&runtimepath)
...
function! python_ftplugin#misc#option#join(values) " {{{1
  " Given a list of strings like the ones returned by
  " `python_ftplugin#misc#option#split()`, this joins the strings together into a
...
function! python_ftplugin#misc#option#split_tags(value) " {{{1
  " Customized version of `python_ftplugin#misc#option#split()` with specialized
...
function! python_ftplugin#misc#option#join_tags(values) " {{{1
  " Customized version of `python_ftplugin#misc#option#join()` with specialized
  " handling for Vim's ['tags' option] [tags].
  let values = copy(a:values)
...
function! python_ftplugin#misc#option#eval_tags(value, ...) " {{{1
  ...
  for pattern in python_ftplugin#misc#option#split_tags(a:value)
    " Make buffer relative pathnames absolute.
    if pattern =~ '^\./'
      let directory = python_ftplugin#misc#escape#substitute(expand('%:p:h'))
      let pattern = substitute(pattern, '^.\ze/', directory, '')
    endif
    " Make working directory relative pathnames absolute.
    if python_ftplugin#misc#path#is_relative(pattern)
      let pattern = python_ftplugin#misc#path#merge(getcwd(), pattern)
    endif
