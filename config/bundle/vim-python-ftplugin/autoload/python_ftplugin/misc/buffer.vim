...
function! python_ftplugin#misc#buffer#is_empty() " {{{1
...
function! python_ftplugin#misc#buffer#prepare(...) " {{{1
  ...
  for bufnr in tabpagebuflist()
    if python_ftplugin#misc#path#equals(options['path'], bufname(bufnr))
  ...
  if !(found || python_ftplugin#misc#buffer#is_empty())
    vsplit
  endif
  ...
  call python_ftplugin#misc#buffer#unlock()
  silent %delete
endfunction

function! python_ftplugin#misc#buffer#lock() " {{{1
  " Lock a special buffer so that its contents can no longer be edited.
  setlocal readonly nomodifiable nomodified
endfunction

function! python_ftplugin#misc#buffer#unlock() " {{{1
