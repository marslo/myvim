...
function! python_ftplugin#misc#timer#start() " {{{1
  " Start a timer. This returns a list which can later be passed to
  " `python_ftplugin#misc#timer#stop()`.
  return s:has_reltime ? reltime() : [localtime()]
endfunction

function! python_ftplugin#misc#timer#stop(...) " {{{1
  ...
  " `python_ftplugin#misc#timer#start()` as an argument.
  "
  " [verbose]: http://vimdoc.sourceforge.net/htmldoc/options.html#'verbose'
  " [printf]: http://vimdoc.sourceforge.net/htmldoc/eval.html#printf()
  if (g:timer_enabled || &verbose >= g:timer_verbosity)
    call call('python_ftplugin#misc#msg#info', map(copy(a:000), 's:convert_value(v:val)'))
  endif
endfunction

function! python_ftplugin#misc#timer#force(...) " {{{1
  ...
  " you pass the list returned by `python_ftplugin#misc#timer#start()` as an argument.
  call call('python_ftplugin#misc#msg#info', map(copy(a:000), 's:convert_value(v:val)'))
endfunction
...
function! s:convert_value(value) " {{{1
  ...
  elseif !empty(a:value)
    if s:has_reltime
      let ts = python_ftplugin#misc#str#trim(reltimestr(reltime(a:value)))
    else
      let ts = localtime() - a:value[0]
    endif
    return python_ftplugin#misc#timer#format_timespan(ts)
  else
    return '?'
  endif
endfunction
...
function! python_ftplugin#misc#timer#format_timespan(ts) " {{{1
