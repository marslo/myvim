...
function! python_ftplugin#misc#list#unique(list) " {{{1
...
function! python_ftplugin#misc#list#binsert(list, value, ...) " {{{1
...
if 0
  " Tests for python_ftplugin#misc#list#binsert().
  let s:list = ['a', 'B', 'e']
  function! s:test(value, expected)
    call python_ftplugin#misc#list#binsert(s:list, a:value, 1)
    if s:list != a:expected
      call python_ftplugin#misc#msg#warn("list.vim: Test failed! Expected %s, got %s",
